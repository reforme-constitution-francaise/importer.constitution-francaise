...
def get_titleArt(el) :
    return el.xpath('text() | b/text() | i/text() | u/text()')

def get_title(div) :
    return get_text(div.xpath('div[@data-role="header"]/h1')[0])


def process_article(filedir,article_id) :
    ...
    set_curr_file(filedir+"/"+titre);
    print_("\n\n## %s " % titre)
    
    content_div = article_div.xpath('div[@data-role="content"]')[0]
    extract_content(filedir,content_div)
  

def extract_content(filedir,content_div) :
        for el in content_div.xpath('.//*[self::h5 or self::p]') :
            if el.tag == "p" :
                print_("\n" + get_text(el))
            elif el.tag == "h5" :
                set_curr_file(get_titleArt(el))
                print_("\n\n## " + get_text(el))
